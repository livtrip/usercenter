package com.trj.usercenter.util.date;
 ...
 * 日期处理
 * @author xierongli
 * @version $$Id: livtripmanager-parent, v 0.1 2016/11/1 14:06 user Exp $$
 */
public class DateProcessor {
    ...
    public static String getShortDate(String baseDate) {
        if (baseDate != null && !baseDate.equals("")) {
            try {
                String wek = getWeekDayName(baseDate);
                String monthString = getMonthName(baseDate);
    ...
    public String converMinToHour(String time_point)
    {
        if(!StringUtils.isNotEmpty(time_point))
        {
            return "";
        }
        int s = Integer.parseInt(time_point) * 5 /60;
    ...
    public static boolean compareDateIncluded(String dateCompare ,String dateStart, String dateEnd)
    {
        Date dateC = fomateDate(dateCompare);
        Date dateS = fomateDate(dateStart);
        Date dateE = fomateDate(dateEnd);
        if(dateC.compareTo(dateS) >= 0 && dateC.compareTo(dateE) <= 0){
            return true;
        }
        return false;
    }

    public static Date setBefroDay(Date date)
    {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.add(Calendar.DATE, -1);
        return fomateDate((new SimpleDateFormat("yyyy-MM-dd")).format(cal.getTime()));
    }

    public static Date setAfterDay(Date date)
    {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.add(Calendar.DATE, 1);
    ...
    public static String[] collectionDate(String[] dates){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String temp="";
        for (int i = 0; i < dates.length - 1; i++) {
            for (int j = i + 1; j < dates.length; j++) {
                try {
                    if (sdf.parse(dates[i]).getTime() - sdf.parse(dates[j]).getTime() > 0) {
                        temp = dates[i];
                        dates[i]=dates[j];
                        dates[j]=temp;
                    }
